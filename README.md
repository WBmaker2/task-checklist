...
- `dist/*`: GitHub Pages 배포 아티팩트(git 미추적)
...
- `npm run build:pages`는 `index.html`, `favicon.ico`, `build/*`를 `dist/`로 모아 GitHub Pages 아티팩트를 만듭니다.
...
### 4) GitHub Pages 배포용 Secrets

GitHub Actions로 Pages를 배포할 경우 저장소 Secrets에 아래 두 값을 넣습니다.

- `FIREBASE_CONFIG_DEV_JSON`
- `FIREBASE_CONFIG_PROD_JSON`

그리고 GitHub Pages의 배포 소스를 `GitHub Actions`로 전환해야 합니다.
