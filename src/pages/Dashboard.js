      ...
      <div>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginBottom: 6 }}>
            ...
            <div>
              <div style={{ fontSize: 15, color: T.text, fontWeight: 600, letterSpacing: 0.5 }}>
          ...
          <h1 style={{ fontFamily: "Outfit,sans-serif", fontSize: 28, fontWeight: 800, color: T.text, margin: "4px 0 0" }}>
            {isToday ? "오늘의 업무" : `${selDate.getMonth() + 1}/${selDate.getDate()} 업무`}
          </h1>
        </div>
        ...
        <div style={{ display: "flex", gap: 20, marginBottom: 28, flexWrap: "wrap", justifyContent: "center" }}>
          ...
          >
            <ProgressRing percent={wPct} size={110} />
            <div style={{ marginTop: 10, fontSize: 13, fontWeight: 600, color: T.text }}>이번 주 진행률</div>
            <div style={{ fontSize: 12, color: T.textMuted }}>
              {wDone} / {wTotal} 완료
            </div>
          </div>

          <div style={{ flex: "1 1 160px", maxWidth: 220, display: "flex", flexDirection: "column", gap: 12 }}>
            <div
              style={{
                background: T.surface,
                borderRadius: 16,
            ...
            >
              <div style={{ fontSize: 32, fontWeight: 800, fontFamily: "Outfit", color: T.accent }}>
                {tDone}
                <span style={{ fontSize: 16, color: T.textMuted }}>/{tTotal}</span>
              </div>
              <div style={{ fontSize: 12, color: T.textMuted, fontWeight: 500 }}>완료</div>
            </div>
            ...
            >
              <div style={{ fontSize: 32, fontWeight: 800, fontFamily: "Outfit", color: pending.length > 0 ? T.danger : T.success }}>
                {pending.length}
              </div>
              <div style={{ fontSize: 12, color: T.textMuted, fontWeight: 500 }}>미완료 업무</div>
            </div>
          </div>
        </div>
        ...
        >
          <h3 style={{ fontSize: 15, fontWeight: 700, margin: "0 0 14px", color: T.text }}>
            📅 이번 주 요약 <span style={{ fontSize: 11, fontWeight: 400, color: T.textMuted }}>· 날짜를 눌러 이동</span>
          </h3>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {weekDates.map((d, i) => {
              ...
              return (
                ...
                >
                  <div style={{ fontSize: 12, fontWeight: 600, color: isSel ? T.accent : T.textMuted }}>{DAYS[i]}</div>
                  <div style={{ fontSize: 11, color: isRT && !isSel ? T.accent : T.textMuted, marginTop: 2, fontWeight: isRT ? 700 : 400 }}>
                    {d.getDate()}일{isRT && !isSel ? " ·오늘" : ""}
                  </div>
                  <div
                    style={{
                      width: 28,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {dayTasks.length === 0 && (
            <div style={{ textAlign: "center", padding: 40, color: T.textMuted, fontSize: 14 }}>등록된 업무가 없습니다 🎉</div>
          )}
        </div>
      </div>
    );
