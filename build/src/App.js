    ...
    const toggle = React.useCallback((tid, ds) => {
      setChecks((prev) => {
        ...
        if (next[k]) {
          ...
          next[k] = (/* @__PURE__ */ new Date()).toISOString();
        }
        return next;
      });
    }, []);
    ...
    const performBackup = React.useCallback(async (options) => {
      ...
      try {
        ...
        setStatus({
          ...
          conflict: false,
          conflictMeta: null,
          message: opts.trigger === "auto" ? "자동 백업이 완료되었습니다." : opts.force ? "강제 업로드가 완료되었습니다." : "클라우드 백업이 완료되었습니다."
        });
...
취소: 로컬 데이터 유지(필요 시 강제 업로드 가능)`
    ...
    React.useEffect(() => {
      ...
      const pollId = window.setInterval(checkLatest, 3e4);
      return () => {
        active = false;
    ...
    React.useEffect(() => {
      ...
      const dueAt = new Date(Date.now() + 6e4).toISOString();
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { minHeight: "100vh", background: T.bg, fontFamily: "'Noto Sans KR',sans-serif", color: T.text } }, /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement(NavBtn, { active: page === "dashboard", icon: "🏠", label: "대시보드", onClick: () => setPage("dashboard") }),
      /* @__PURE__ */ React.createElement(NavBtn, { active: page === "checklist", icon: "✅", label: "체크리스트", onClick: () => setPage("checklist") }),
      /* @__PURE__ */ React.createElement(NavBtn, { active: page === "manage", icon: "⚙️", label: "업무관리", onClick: () => setPage("manage") }),
      /* @__PURE__ */ React.createElement(NavBtn, { active: page === "stats", icon: "📊", label: "통계", onClick: () => setPage("stats") }),
      /* @__PURE__ */ React.createElement(NavBtn, { active: page === "backup", icon: "☁️", label: "백업", onClick: () => setPage("backup") })
    ));
  }
  window.App = App;
})();
